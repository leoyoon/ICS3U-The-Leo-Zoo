...
/**
 * @TODO 	work on patterns & board clear
 *
 */
...
public class Zoo extends JFrame implements ActionListener
{
	...
	private Font hiddenFont = new Font("Arial", Font.BOLD, 0);	
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		for (int i = 0; i < playingField.length; i++)
		{
			if (e.getSource() == playingField[i])
			{
				...
				if (playingField[i].getText().equalsIgnoreCase("true"))
				{
				...
				else
				{
					...
					playingField[i].setFont(new Font("Trebuchet MS", Font.BOLD, 72));
				}	
				...
				if (rescueTries == 0) 
				{
					...
					{
						playingField[j].setEnabled(false);
					}
	...
	{
		animalRandomNumber = (int)(Math.random()*50 + 1);
	...
	private void makePattern(String s)
	{
		...
		case "Sheep": 	
			...
			int midTile = (rowNum - 1) * 5 + 2;
			
			playingField[midTile].setText("true");
			playingField[midTile].setFont(hiddenFont);
			playingField[midTile].setForeground(farmColour);
			
			playingField[midTile - 1].setText("true");
			playingField[midTile - 1].setFont(hiddenFont);
			playingField[midTile - 1].setForeground(farmColour);
			...
			{
				playingField[midTile + 2].setText("true");
				playingField[midTile + 2].setFont(hiddenFont);
				playingField[midTile + 2].setForeground(farmColour);
			}
			else
			{
				playingField[midTile - 2].setText("true");
		...
		case "Rabbit":
			...
			int firstTile = (int)(Math.random()*10 + 1);
			
			for (int i = 0; i < 4; i++)
			{
				playingField[firstTile + (i * 5)].setText("true");
				playingField[firstTile + (i * 5)].setFont(hiddenFont);
				playingField[firstTile + (i * 5)].setForeground(farmColour);
			}
